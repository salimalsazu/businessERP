...
const createUserService = async (req: Request): Promise<IGenericResponse> => {
  const response: IGenericResponse = await HttpService.post('/auth/create-user', req.body, {
    headers: {
      //@ts-ignore
...
const updateUserService = async (req: Request): Promise<IGenericResponse> => {
  const { id } = req.params;
  const response: IGenericResponse = await HttpService.patch(`/users/update-user/${id}`, req.body, {
    headers: {
      //@ts-ignore
      Authorization: req.headers.authorization
    }
